...
while True:
    ...
    results = pose.process(cv.cvtColor(frame, cv.COLOR_BGR2RGB)) # mediapipeに処理を渡す
    ...
    for n in range(len(posName)): # 上記で設定したパーツ分ループする
        ...
        cv.circle(frame, (x, y), 15, (0, 255, 0), thickness = 2)
        # print(x, y) # デバッグ用

    cv.imshow("Frame pose", frame) # 円描画した結果の表示
