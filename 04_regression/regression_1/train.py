...
datasets_raw = ld.ImageFolder_reg1(dataset_path, cf.transforms_train) # データの読み込み
...
criterion = nn.L1Loss() # 外れ値が少ないような対象なら MSELoss もよい
calc_acc = nn.L1Loss()
...
def Train_Eval(model, criterion, optimizer, data_loader, device, epoch, max_epoch, is_val = False):
    ...
    for n, (data, label) in enumerate(data_loader): # バッチ毎にデータ読み込み
        ...
        if is_val == False:
            disp_score_t = f"{epoch + 1:03} / {max_epoch:03} [ {n + 1:04} / {len(data_loader):04} ] l: {total_loss / (n + 1):.05f} a: {cf.val_rate * total_acc / (n + 1):.03f}"
            print(f"\r {disp_score_t}", end = "")
        else: 
            print(f"\r {disp_score_t} l: {total_loss / (n + 1):.05f} a: {cf.val_rate * total_acc / (n + 1):.03f}", end = "")
    ...
    return total_loss / (n + 1), cf.val_rate * total_acc / (n + 1)
