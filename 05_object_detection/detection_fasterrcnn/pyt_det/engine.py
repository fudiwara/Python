...
def train_one_epoch(model, optimizer, data_loader, device, epoch, print_freq, scaler=None):
    ...
    optimizer.zero_grad() # ループ開始前に勾配を初期化
    for i, (images, targets) in enumerate(metric_logger.log_every(data_loader, print_freq, header, True)):
        ...
        with torch.autocast(device_type=device, enabled=scaler is not None):
            ...
            loss_for_backward = losses / cf.accumulation_steps # 平均勾配にするため損失をステップ数で割る
        ...
        if scaler is not None: # 逆伝播：勾配の蓄積
            scaler.scale(loss_for_backward).backward()
        else:
            loss_for_backward.backward()

        if (i + 1) % cf.accumulation_steps == 0 or (i + 1) == len(data_loader):
