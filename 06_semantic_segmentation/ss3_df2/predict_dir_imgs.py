...
for f in range(len(fileList)):
    if fileList[f].is_file() and (fileList[f].suffix in exts): # ファイルのみ処理する
        ...
        for i in range(len(scores)):
            ...
            cv.rectangle(img, (x0, y0 + 2), (x0 + t_w, y0 + t_h + 2), cf.box_col[prd_cls], thickness = -1) # テキストの背景の矩形
            cv.putText(img, text_val, (x0, y0 + 13), cv.FONT_HERSHEY_DUPLEX, font_scale, (255, 255, 255), 1, cv.LINE_AA)
