...
# def Train_Eval(model,criterion,optimizer,data_loader,device,epoch,max_epoch,is_val = False):
    ...
    for n, (data, lbl_0, lbl_1) in enumerate(data_loader): # バッチ毎にデータ読み込み
        ...
        if is_val == False:
            disp_score_t = "{:03} / {:03} [ {:04} / {:04} ] l: {:.05f} a0: {:.03f} a1: {:.03f}".format(epoch+1, max_epoch, n + 1, len(data_loader), total_loss/(n+1), cf.val_rate_0 * total_acc_0/counter, cf.val_rate_1 * total_acc_1/counter)
            print("\r {}".format(disp_score_t), end = "")
        else: 
            print("\r {} l: {:.05f} a0: {:.03f} a1: {:.03f}".format(disp_score_t, total_loss/(n+1), cf.val_rate_0 * total_acc_0/counter, cf.val_rate_1 * total_acc_1/counter), end = "")
...
# torch.save(model.state_dict(), f"{log_dir}/_m_{id_str}_{epoch:03}.pth") # モデルの保存
