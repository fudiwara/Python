...
while True:
    ...
    # if False:
        img = Image.fromarray(cv.cvtColor(frame, cv.COLOR_BGR2RGB)) # OpenCV形式からPIL形式へ変換
        ...
        for i in range(len(scores)):
            ...
            cv.rectangle(frame, p0, p1, box_col, thickness = 2) # 検出領域の矩形
            cv.rectangle(frame, (x0, y0 - t_h), (x0 + t_w, y0), box_col, thickness = -1) # テキストの背景の矩形
