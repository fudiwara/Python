...
datasets_raw = ld.ImageFolder_reg1(dataset_path, cf.data_transforms) # データの読み込み
...
# def Train_Eval(model,criterion,optimizer,data_loader,device,epoch,max_epoch,is_val = False):
    ...
    for n, (data, label) in enumerate(data_loader): # バッチ毎にデータ読み込み
        ...
        if is_val == False:
            disp_score_t = "{:03} / {:03} [ {:04} / {:04} ] l: {:.05f} a: {:.03f}".format(epoch+1, max_epoch, n + 1, len(data_loader), total_loss/(n+1), cf.val_rate * total_acc/counter)
            print("\r {}".format(disp_score_t), end = "")
        else: 
            print("\r {} l: {:.05f} a: {:.03f}".format(disp_score_t, total_loss/(n+1), cf.val_rate * total_acc/counter), end = "")
