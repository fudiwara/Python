...
options = mp.tasks.vision.ImageSegmenterOptions( # セグメンテーションのオプション
    base_options = mp.tasks.BaseOptions(model_asset_path = "models/selfie_multiclass_256x256.tflite"), # 人の他クラス用
...
while True:
    ...
    segmented_colored_image = color_map[category_masks]
    img_disp = cv.addWeighted(frame, 0.6, segmented_colored_image, 0.4, 0) # 全体のマスクと入力フレームを合成
    ...
    cv.imshow("segmented mask", segmented_colored_image) # 色づけされたマスクの表示
    cv.imshow("segmented result", img_disp) # 結果の表示
